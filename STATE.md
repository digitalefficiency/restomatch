...
**Next planned task:** Pilot gate (plan §8a). Remaining: Phase 2 cred-gated activation (Supabase/OCR/Resend/observability + CI) when keys arrive; live QA/design-review browser pass on the new UI; optional per-IP Redis rate-limit on leads.create.
**Open blockers:** none
**Tests at end of session:** 376 passing (69 matching + 29 catalog + 24 ocr + 17 procurement + 18 charts + 209 api + 5 db + 5 billing) + 2 E2E; `next build` green
