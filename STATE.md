...
**Active milestone:** Production master plan (see ~/.claude/plans/lovely-scribbling-pony.md). Phases 1 (security), 3 (entitlements), and **4 (admin panel) COMPLETE**. Phase 2 (production wiring) PARKED pending credentials — see memory `restomatch-phase2-parked` + docs/REMAINING-SESSIONS.md.
**Last completed task:** Phase 4 — `users.is_platform_admin` (migration 0007), `adminProcedure` (cross-tenant on owner connection via `ctx.adminDb`, gated by flag OR PLATFORM_ADMIN_EMAILS allowlist requiring verified email), `adminRouter` (listRestaurants/getRestaurant/assignPlan/setOverrides/setMemberRole/listLeads — assignPlan wires the Phase-3 billing-account+subscription flow, tx-locked + idempotent), admin UI (`apps/web/app/admin/*`), audit-logged mutations. 16-agent review: privilege-escalation / adminDb-wiring / cross-tenant scoping all survived; fixed HIGH (assignPlan concurrency → duplicate accounts) + last-owner-demotion guard + 0-limit-override reject + emailVerified guard + audit before-images.
**Next planned task:** Pilot gate (plan §8a) is now reachable. Remaining for pilot: Phase 2 cred-gated activation (Supabase/OCR/Resend/observability + CI) when keys arrive. Then Phases 5–7 (UX polish incl. the deferred entitlement upgrade-CTA, marketing landing, GTM doc) during the pilot.
**Open blockers:** none (Phase 2 cred-gated items written ready-to-activate; parked in memory)
**Tests at end of session:** 353 passing (69 matching + 29 catalog + 24 ocr + 17 procurement + 18 charts + 186 api + 5 db + 5 billing) + 2 E2E
