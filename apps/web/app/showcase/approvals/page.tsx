  ...
  return (
    <main ref={containerRef} className="max-w-6xl mx-auto px-6 py-12" dir="rtl">
      <div className="mb-10">
        ...
        <h1 className="text-4xl font-bold tracking-tight mb-2 text-slate-900">מטריצת אישורים</h1>
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/90 backdrop-blur-xl overflow-hidden shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_-8px_rgba(15,23,42,0.06)]">
        <div className="grid grid-cols-[40px_1fr_140px_160px_120px_60px] gap-4 px-5 py-3 border-b border-slate-200/70 text-xs uppercase tracking-wider text-slate-500 font-semibold bg-slate-50/50">
          <div>#</div>
          <div>סוג חריגה</div>
          <div>תפקיד מאשר</div>
      ...
      <div className="mt-6 text-xs text-slate-500 flex items-center gap-4 flex-wrap">
...
function RuleRow({ rule, idx }: { rule: Rule; idx: number }) {
  ...
  const toneStyles: Record<SeverityTier, string> = {
    ...
    'review-manager': 'bg-blue-50/30 hover:bg-blue-50/60 border-r-blue-300',
  ...
  return (
    <div
      className={`rule-row border-b border-slate-100 last:border-b-0 border-r-2 ${toneStyles[rule.severity]}`}
    >
      <button
        type="button"
      ...
      >
        <div className="text-xs text-slate-400 tabular-nums font-bold">
        ...
        <div className="text-center">
          <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-md bg-white border border-slate-200 text-xs font-bold tabular-nums text-slate-700">
...
function AuditLog({ entries }: { entries: AuditEntry[] }) {
  return (
    <div className="bg-slate-50/80 border-t border-slate-200/70 px-5 py-5">
...
function AuditEntryRow({ entry }: { entry: AuditEntry }) {
  const decisionColors = {
    ...
    auto: 'text-blue-700 bg-blue-50 border-blue-200',
  } as const;
  ...
  return (
    <div className="grid grid-cols-[140px_1fr_120px_100px_140px] gap-3 items-center text-xs py-2 px-3 rounded-lg bg-white border border-slate-200/70 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
      ...
      <div>
        ...
        {entry.note ? <div className="text-[11px] text-slate-500 mt-0.5">{entry.note}</div> : null}
      </div>
      <div>
        <span
      ...
      <div
        className="flex items-center gap-1.5 text-slate-500 font-mono text-[10px] tabular-nums"
        title={entry.hash}
      >
...
function RoleBadge({ role, small }: { role: ApproverRole; small?: boolean }) {
  const map: Record<ApproverRole, { label: string; icon: React.ReactNode; tint: string }> = {
    auto: {
      ...
      tint: 'text-slate-600 bg-slate-100 border-slate-200',
    },
    clerk: {
      label: 'מקבל',
...
function LegendSwatch({ tone, label }: { tone: SeverityTier; label: string }) {
  const colors: Record<SeverityTier, string> = {
    ...
    'review-manager': 'bg-blue-100 border-blue-300',
