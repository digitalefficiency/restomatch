  ...
  return (
    <main ref={containerRef} className="max-w-7xl mx-auto px-6 py-10 space-y-8" dir="rtl">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm text-slate-500 font-medium ml-2">סנן:</span>
        <FilterChip
          active={filter === 'all'}
      ...
      <div className="grid gap-3">
        ...
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-10 text-center text-slate-500">
...
function KpiCard({
  ...
  return (
    <div className="audit-kpi rounded-2xl border border-slate-200/70 bg-white/90 backdrop-blur-xl p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_-12px_rgba(15,23,42,0.06)]">
      ...
      <div className="text-3xl font-bold tabular-nums text-slate-900 mb-1">{value}</div>
  ...
  return (
    <button
      ...
      className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${
        active
          ? 'bg-blue-600 border-blue-600 text-white shadow-[0_2px_8px_rgba(37,99,235,0.25)]'
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="text-left">
          <div className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">חויב</div>
...
function statusBadge(status: InvoiceAuditStatus): { label: string; tint: string } {
  switch (status) {
    ...
    case 'minor':
      return { label: 'הערה', tint: 'bg-slate-50 text-slate-700 border-slate-200' };
    case 'major':
